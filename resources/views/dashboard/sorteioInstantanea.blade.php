...
<div class="card-content">
    ...
    <table id="aposta" class="mdl-data-table" cellspacing="0" width="100%">
        ...
        <tbody>
        @forelse($data as $sorteio)
            <tr>
                <td>{{$sorteio->numpule}} </td>
                <td>{{$sorteio->sorteio}}</td>
                <td>{{$sorteio->nomreven}}</td>
                <td>{{$sorteio->datsor}}</td>
                <td>{{$sorteio->horsor}}</td>
                <td>R$ {{ number_format($sorteio->vlrpremio, 2, ',', '.') }}</td>
